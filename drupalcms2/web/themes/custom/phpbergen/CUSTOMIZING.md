## Customizing Phpbergen

**Don't subtheme Phpbergen!** It does not provide backwards compatibility. This allows us to rapidly innovate, iterate, and improve it. If you create a sub-theme of Phpbergen, it is likely to break in the future.
...
If you want to make deeper customizations (e.g., to components or JavaScript), you will need to convert Phpbergen to a custom theme with the same machine name. You can do this by running the following at the command line, from the Drupal project root (assuming `web` is the web root):

```shell
mkdir -p web/themes/custom
cp -R web/themes/contrib/phpbergen web/themes/custom/phpbergen
git add web/themes/custom/phpbergen
composer remove drupal/phpbergen
```
...
Phpbergen uses [single-directory components](https://www.drupal.org/docs/develop/theming-drupal/using-single-directory-components) and comes with a variety of commonly used components. You can add new components and modify existing ones, but be sure to rebuild the CSS when you make changes.

## Building CSS

Phpbergen uses [Tailwind](https://tailwindcss.com) to simplify styling by using classes to compose designs directly in the markup.
...
Phpbergen uses [Prettier](https://prettier.io) to automatically format code for consistency. The project is configured with plugins for Tailwind CSS and Twig templates.
...
All the objects created this way will be stored in a global variable so you can do stuff with them later. Since the `namespace` variable at the top of component.js is `phpbergenComponents`, you would find the Accordion's ComponentType instance at `window.phpbergenComponents.accordion`.

Furthermore, `window.phpbergenComponents.accordion.instances` is an array of all the ComponentInstance objects, and `window.phpbergenComponents.accordion.elements` is an array of all the component container elements.
